...
Live: [agora.market](https://agora.market)
...
A Sepolia subgraph (`subgraph/`) indexes the full protocol surface:
...
`ProposalCreated` spawns per-proposal templates for lifecycle events; prices follow the contract convention (USDC 6d per 1e18 outcome token). The subgraph is the data source for the **Agora copilot** (`backend/src/services/copilotService.js`, `frontend/components/copilot-panel.tsx`).
